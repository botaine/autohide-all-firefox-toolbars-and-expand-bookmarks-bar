...
(function () {
  function init() {
    ...
    // True while "fullscreen mode" is active (fake-fullscreen.uc.js's
    // toggle, or real OS-level fullscreen as a safety net) - the same
    // two attributes userChrome.css's own section 3e keys off to keep
    // all three toolbars permanently visible in fullscreen. An expanded
    // folder auto-collapsing itself while the toolbar it lives on can
    // never auto-hide would be a visible inconsistency, so this is
    // checked below alongside the existing hover/focus-within/forceopen
    // guard.
    function isFullscreenMode() {
      const root = document.documentElement;
      return (
        root.getAttribute("data-uc-fakefullscreen") === "true" ||
        root.getAttribute("sizemode") === "fullscreen"
      );
    }
    ...
    if (navigatorToolbox) {
      navigatorToolbox.addEventListener("mouseleave", () => {
        ...
        collapseAllTimer = setTimeout(() => {
          ...
          if (navigatorToolbox.matches(":hover, :focus-within") || navigatorToolbox.hasAttribute("forceopen") || isFullscreenMode()) return;
          collapseAllFolders();
        }, delayMs);
      });
